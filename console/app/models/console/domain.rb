...
class Domain < RestApi::Base
  ...
  on_exit_code(158, :on_invalid => (UserAlreadyHasDomain = Class.new(ActiveResource::ResourceInvalid)))
  on_exit_code(103, :on_invalid => (AlreadyExists = Class.new(ActiveResource::ResourceInvalid)))
  ...
  def self.find_one(options)
    ...
    raise RestApi::ResourceNotFound.new(self, nil) if domain.nil?
    domain
  end
end
end
