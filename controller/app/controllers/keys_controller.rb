class KeysController < BaseController
  ...
  def update
    ...
    if @cloud_user.ssh_keys.where(name: id).count == 0
      ...
      @reply = RestReply.new(requested_api_version, :not_found)
    ...
    begin
      ...
      ssh_key = RestKey.new(key, get_url, nolinks)
      log_action(@request_id, @cloud_user._id, @cloud_user.login, "UPDATE_KEY", true, "Updated SSH key #{id}", 'IP' => request.remote_ip)
      @reply = RestReply.new(requested_api_version, :ok, "key", ssh_key)
      ...
      @reply = RestReply.new(requested_api_version, :internal_server_error)
