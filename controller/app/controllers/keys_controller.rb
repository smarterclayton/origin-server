class KeysController < BaseController
  ...
  def show
    ...
    begin
      key = current_user.ssh_keys.accessible(current_user).find_by(name: id)
  ...
  def create
    ...
    begin
      result = current_user.add_ssh_key(key)
  ...
  def update
    ...
    # validate the key name using regex to avoid a mongo call, if it is malformed
    if id !~ SshKey::KEY_NAME_COMPATIBILITY_REGEX or current_user.ssh_keys.accessible(current_user).where(name: id).empty?
      return render_error(:not_found, "SSH key '#{id}' not found", 118)
    end
    ...
    begin
      result = current_user.update_ssh_key(key)
      ssh_key = RestKey.new(key, get_url, nolinks)
      render_success(:ok, "key", ssh_key, "Updates SSH key #{id} for user #{current_user.login}", result, nil, 'IP' => request.remote_ip)
    rescue OpenShift::LockUnavailableException => e
      return render_error(:service_unavailable, "User is currently busy performing another operation. Please try again in a minute.", e.code)
    rescue Exception => e
      return render_exception(e)
